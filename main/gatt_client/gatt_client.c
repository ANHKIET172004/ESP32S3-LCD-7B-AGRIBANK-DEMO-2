...
//#include "D:/ESP32/ESP32S3_LCD 7B_DEMO_3/ESP32-S3-Touch-LCD-7B-Demo/ESP32-S3-Touch-LCD-7B-Demo/ESP-IDF/16_LVGL_UI/main/ui/ui_wifi_Screen.c"
 ...
 void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_SEARCH_RES_EVT: {
        ...
        ESP_LOGI(GATTC_TAG, "start_handle %d end_handle %d current handle value %d", p_data->search_res.start_handle, p_data->search_res.end_handle, p_data->search_res.srvc_id.inst_id);
    ...
    case ESP_GATTC_SEARCH_CMPL_EVT:
        ...
        if (get_server){
            ...
            if (count > 0){
                char_elem_result = (esp_gattc_char_elem_t *)malloc(count * sizeof(esp_gattc_char_elem_t));
                if (!char_elem_result){
    ...
    case ESP_GATTC_REG_FOR_NOTIFY_EVT: {
        ...
        if (p_data->reg_for_notify.status != ESP_GATT_OK){
            ...
            if (count > 0){
                descr_elem_result = malloc(count * sizeof(esp_gattc_descr_elem_t));
    ...
    case ESP_GATTC_NOTIFY_EVT:
        if (p_data->notify.is_notify){
            ESP_LOGI(GATTC_TAG, "***** RECEIVED NOTIFICATION *****");
        }else{
            ESP_LOGI(GATTC_TAG, "***** RECEIVED INDICATION *****");
        }
        
        // In tin nhắn nhận được từ server
        char received_msg[256] = {0};
        memcpy(received_msg, p_data->notify.value, p_data->notify.value_len);
        ESP_LOGI(GATTC_TAG, "Response from server: %s", received_msg);
        break;
    case ESP_GATTC_WRITE_DESCR_EVT:
        ...
        // Kết nối thành công, bắt đầu gửi tin nhắn
        connect = true;
        ESP_LOGI(GATTC_TAG, "***** CONNECTION ESTABLISHED - READY TO SEND MESSAGES *****");
        break;
    ...
    case ESP_GATTC_WRITE_CHAR_EVT:
        ...
        ESP_LOGI(GATTC_TAG, "***** MESSAGE SENT SUCCESSFULLY *****");
        break;
 ...
 void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    ...
    case ESP_GAP_BLE_SCAN_START_COMPLETE_EVT:
        ...
        ESP_LOGI(GATTC_TAG, "***** SCANNING FOR ESP32-Server *****");
        break;
    case ESP_GAP_BLE_SCAN_RESULT_EVT: {
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT:
            ...
            if (adv_name != NULL) {
                if (strlen("ESP32-Server") == adv_name_len && strncmp((char *)adv_name, "ESP32-Server", adv_name_len) == 0) {
                    ESP_LOGI(GATTC_TAG, "***** FOUND ESP32-Server - CONNECTING *****");
                    if (connect == false) {
                        connect = true;
...
*/
    ...
    while(1) {
        if (connect) {
            ...
            ESP_LOGI(GATTC_TAG, "***** SENDING MESSAGE: %s *****", message);
            ...
            vTaskDelay(1000 / portTICK_PERIOD_MS); // Gửi tin nhắn mỗi 5 giây
        }//
          else {//
           ...
           // }//
            ...
            vTaskDelay(1000 / portTICK_PERIOD_MS); // Chờ kết nối
